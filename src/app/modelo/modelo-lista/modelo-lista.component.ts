...
export class ModeloListaComponent implements OnInit {
  ...
  modelosLazyLoad = new Array<Modelo>()
...
title = 'Modelos'
  ...
  loadCustomers(event: LazyLoadEvent) {
    ...
    setTimeout(() => {
      if (this.modelos) {
        ...
        this.modelos = [...this.modelosLazyLoad.slice(numPrimeiraLinha, numLinhasPagina)];
  ...
  onDelete(id: number) {
    this.confirmationService.confirm({
      message: 'Deseja realmente DELETAR esse Modelo?',
      header: 'DELETAR',
      ...
      accept: () => {
        ...
        this.modeloService.getExcluir(id).subscribe(
          () => {
            this.messageService.add({
              key: 'bc',
              severity: 'success',
              summary: 'Deletado',
              detail: 'Modelo deletado com sucesso!'
            });
            setTimeout(() => {
